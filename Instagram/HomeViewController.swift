...
class HomeViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if FIRAuth.auth()?.currentUser != nil {
            ...
            if observing == true {
                ...
                // FIRDatabaesのobseveEventが上記コードにより解除されたためfalseとする
                observing = false
            }
        }
    ...
    func handleButton(sender: UIButton, event:UIEvent) {
        ...
        if let uid = FIRAuth.auth()?.currentUser?.uid {
            if postData.isLiked {
                // すでに言い値をしていた場合は言い値を解除するためIDを取り除く
                var index = -1
    ...
    func handleCommentButton(sender: UIButton, event:UIEvent) {
        ...
        let commentViewController
            = storyboard?.instantiateViewController(withIdentifier: "Comment") as! CommentViewController
        commentViewController.postData = postArray[sender.tag]
